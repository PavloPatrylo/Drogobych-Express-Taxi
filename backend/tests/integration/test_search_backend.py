...
async def test_search(db_session):
    ...
    На вхід:
        ...
        - База даних з рейсами через db_session.
    ...
    trips_naive = (await db_session.execute(stmt_naive)).scalars().all()
    ...
    trips_kyiv = (await db_session.execute(stmt_kyiv)).scalars().all()
